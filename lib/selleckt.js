 ...
 *
 * Dependencies: jQuery, underscore.js, handlebars.js
 *
 * License: MIT
**/
    ...
    */
    ...
    if(typeof define === 'function' && define.amd){
        ...
        define(['jquery', 'underscore', 'handlebars'], function($, _, Handlebars){
            return factory($ || root.jQuery, _ || root._, Handlebars || root.Handlebars, MicroEvent);
        });
    }else{
        root.selleckt = factory(root.jQuery, root._, root.Handlebars, MicroEvent);
    }

})(this, function($, _, Handlebars, MicroEvent){
    ...
    if(!Handlebars){
        throw new Error('selleckt requires handlebars to be loaded');
    }
    ...
    TEMPLATES = {
        SINGLE:
            ...
            '</div>',
        MULTI:
            '<div class="{{className}} {{multiClassName}}" tabindex=1>' +
    ...
    function SingleSelleckt(options){
        ...
        this.multiClassName = settings.multiClassName;
    ...
    function parseTemplate(template) {
        if (typeof(template) === 'function') {
            ...
            throw new Error('Please provide a valid handlebars template.');
        }
    }
    ...
    _.extend(SingleSelleckt.prototype, {
        ...
        _parseItemsFromOptions: function($selectEl){
            ...
            return _($selectEl.find('option')).reduce(function(memo, option){
                ...
                if ( $option.is( '[value][value!=""]' ) ) {
                    if($option.is(':selected') ){
                        memo.selectedItems.push(item);
                    }
                    memo.items.push(item);
                } else if ( $option.is( '[value=""],:not([value])' ) && $option.is(':first-child' ) ) {
                    // use the first option as placeholder if it doesn't have a value attribute
                    that.placeholderText = $option.text();
                }
        ...
        getTemplateData: function() {
            ...
            return _.extend(this.mainTemplateData, {
                ...
                multiClassName : this.multiClassName,
    ...
    MultiSelleckt.prototype.selectItem = function(item){
        ...
        if ( this.selectedItems.length == this.$items.find('li').length ) {
            // if we're out of options, hide the select box
            this.$sellecktEl.find('.'+this.selectedClass).hide();
        }
        this.trigger('itemSelected', item);
    };
    ...
    MultiSelleckt.prototype.render = function(){
        ...
        // if all options are selected, hide the dropdown menu 
    ...
    MultiSelleckt.prototype.removeItem = function(item){
        ...
        // show the sellect dropdown in case it's hidden
        this.$sellecktEl.find('.'+this.selectedClass).show();
